...
function request(options){
	...
	if(options.query && typeof options.query === "object"){
		...
		if(!Array.isArray(options.query)){
			...
			queryArr = queryArr.map(function(param){ return param === null ? undefined : param; });
		}
		...
		for(var i = 0, l = queryArr.length; i < l; i += 2){
			...
			if(options.queryIsEncoded){
				query += queryArr[i] + "=" + queryArr[i + 1];
			}else{
				query += escapeQueryValue(queryArr[i]) + "=" + escapeQueryValue(queryArr[i + 1]);
			}
		}
