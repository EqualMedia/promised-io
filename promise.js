...
/**
 * Amount of time to wait before emitting for unhandled errors. Set to 0 by default, which means unhandled
 * errors are not detected. This is recommended for production.
 * @property
 * @type integer
 */
exports.detectUnhandled = 0;
...
function scheduleUnhandledTest(id, error){
	setTimeout(function(){
 ...
 * @param {function} [canceller] If defined, the promise becomes cancellable and will expose a "cancel" method. When cancelled, this function is invoked and can return an error value.
 */
exports.Deferred = function(canceller){
	if(exports.detectUnhandled){
		var id = this["-id-"] = "" + DeferredCount++;
	}
	...
	this.reject = function(error, ignoreUnhandled){
		isError = true;
		notifyAll(error);
		if(!ignoreUnhandled && !handled && exports.detectUnhandled){
			scheduleUnhandledTest(id, error);
		}
		return handled;
	};
		...
		if(func){
			...
			if(isError){
				if(listener.deferred.reject(result, true)){
					handled = true;
				}else if(exports.detectUnhandled){
					UnhandledDeferreds[listener.deferred["-id-"]] = [];
					if(!(id in UnhandledDeferreds)){
					  UnhandledDeferreds[id] = [];
					}
					UnhandledDeferreds[id].push(listener.deferred["-id-"]);
				}
			}else{
